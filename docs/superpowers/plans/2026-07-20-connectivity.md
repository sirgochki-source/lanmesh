...
- Полный прогон после каждой задачи: `go test ./...` — обязан быть зелёным.
...
- [ ] **Шаг 5: Коммит отбраковки отдельно**

```bash
git add internal/portmap/
git commit -m "feat(portmap): правило отбраковки непригодных внешних адресов

Отдельным коммитом до самих протоколов: это единственное место, где фича
может сделать ХУЖЕ, чем её отсутствие. Роутер за операторским NAT честно
отдаст WAN-адрес из 100.64/10, при двойном NAT — приватный; анонс такого
адреса заставит пиров долбиться в мусор вместо рабочих кандидатов."
```

- [ ] **Шаг 6: Реализовать NAT-PMP (самый простой из трёх)**
