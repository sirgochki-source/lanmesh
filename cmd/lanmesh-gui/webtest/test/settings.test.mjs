...
test('settingsBody: галка проброса порта отражает portMapEnabled и показывает статус', () => {
  const on = settingsBody({ running: false, portMapEnabled: true, portmap: 'работает: 203.0.113.5:31337 (upnp)' });
  assert.match(on, /data-act="portmap"/);
  assert.match(on, /id="s-portmap"[^>]*checked/);
  assert.match(on, /работает: 203\.0\.113\.5:31337 \(upnp\)/);

  const off = settingsBody({ running: false, portMapEnabled: false, portmap: 'выключен' });
  assert.doesNotMatch(off, /id="s-portmap"[^>]*checked/);
  assert.match(off, /выключен/);
});

test('renderSettingsCompact — настройки без отдельной кнопки «назад» (возврат через ⚙)', () => {
  const s = renderSettingsCompact({ running: false, sendLogs: true });
  assert.match(s, /сигналк/i);
  assert.match(s, /диагностик/i);
  assert.match(s, /Настройки/);
  assert.doesNotMatch(s, /data-act="show-list"/);
});
