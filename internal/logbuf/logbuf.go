...
// maxPartialLen — потолок незавершённого (без \n) хвоста, чтобы поток без переводов
// строк не раздул память. 64 КБ заведомо больше любой осмысленной строки лога.
...
func (b *Buffer) Write(p []byte) (int, error) {
	...
	// Хвост без \n не должен расти без предела: обычный log пишет строку целиком, но
	// чужой io.Writer (бинарь/сторонний логгер без переводов строк) иначе тихо съел
	// бы память. Сверх потолка обрезаем начало — это уже мусор для диагностики.
	if len(b.partial) > maxPartialLen {
		b.partial = b.partial[len(b.partial)-maxPartialLen:]
	}
